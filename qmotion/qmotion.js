...
module.exports = function(RED) {
  ...
  function validateMsg(node, msg) {
    var validMsg = true;
    var msgProperty = ["channel", "blindPosition"];
    ...
    if (!(typeof msg.payload === "object")) {
      ...
      for (i in msgProperty) {
        if (!(msgProperty[i] in msg.payload)) {
          node.error(
            "qmotion.error.property-" + msgProperty[i] + "-missing",
            msg
          );
          validMsg = false;
        }
      ...
      if (validMsg) {
        if (typeof msg.payload.channel != "number") {
          node.error(
            "qmotion.error.invalid-channel: " + msg.payload.channel,
            msg
          );
          validMsg = false;
        }
        if (
          ...
          node.error(
            "qmotion.error.invalid-blindChannel: " + msg.payload.blindPosition,
            msg
          );
          validMsg = false;
        }
